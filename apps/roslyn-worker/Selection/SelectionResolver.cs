...
{
    public List<PirNode> ResolveRange(
    ...
    )
    {
        ...
        foreach (
            VariableDeclaratorSyntax variable in root.DescendantNodes()
                .OfType<VariableDeclaratorSyntax>()
        )
        {
            if (!selectionSpan.Contains(variable.Span))
                continue;
        ...
        {
            if (!selectionSpan.Contains(node.Span))
                continue;
            ...
            {
                AddPirNode(node, mapper, selectedNodes);
            }
        }
    ...
    )
    {
         ...
         */
        LocalDeclarationStatementSyntax? localDeclaration = root.DescendantNodes()
         ...
         */
        SyntaxNode? declaration = root.DescendantNodesAndSelf()
            .Where(node =>
        ...
        {
            ResolveDeclaration(declaration, mapper, selectedNodes);
